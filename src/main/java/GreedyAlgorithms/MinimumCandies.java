...
//        int[] ratings= {1,2};
        ...
        int[] left_choc = new int[ratings.length];
        left_choc[0] = 1;
        for(int i=1; i<ratings.length; i++){
            if(ratings[i] > ratings[i-1]){
                left_choc[i] = left_choc[i-1]+1;
            }
            else left_choc[i] = 1;
        }
        int[] right_choc = new int[ratings.length];
        right_choc[ratings.length-1] = 1;
        for(int i = ratings.length-2; i>=0; i--){
            if(ratings[i] > ratings[i+1]){
                right_choc[i] = right_choc[i+1]+1;
            }
            else right_choc[i] = 1;
        }
        ...
        for(int i=0; i<ratings.length; i++){
            int ch = Math.max(left_choc[i],right_choc[i]);
            total_choc += ch;
        }
